...
function renderSupportTab() {
  ...
  if (supportTab === "home") {
    ...
    content.innerHTML = tickets.length
      ? tickets.map(t => `
        <div class="bg-orange-50 rounded-lg p-4 mb-4 shadow border border-orange-200">
          ...
          <button class="text-blue-600 hover:underline text-xs font-semibold" onclick="showSupportChat('${t.id}')">View Conversation</button>
        </div>
    ...
    content.innerHTML = tickets.length
      ? tickets.map(t => `
        <div class="bg-gray-50 rounded-lg p-4 shadow text-center mb-4">
          <div class="text-lg font-semibold text-gray-700 mb-1">${t.title}</div>
          <div class="text-gray-500 mb-4">${t.status}</div>
          <button onclick="showSupportChat('${t.id}')" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700">View Conversation</button>
        </div>
