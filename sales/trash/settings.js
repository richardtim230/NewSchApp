...
let currentProfile = {};
let accountSettings = {};
let mode = "edit"; // "view" or "edit"

// Helper to enable/disable form fields
function setFormMode(mode) {
  const editableIds = ['bank', 'accountName', 'accountNumber', 'idType', 'idDocument', 'proofOfAddress'];
  editableIds.forEach(id => {
    const el = document.getElementById(id);
    if (el) {
      el.disabled = (mode === 'view');
      if (id === 'idDocument' || id === 'proofOfAddress') {
        el.required = (mode === 'edit');
      }
    }
  });
  document.getElementById('saveBtn').style.display = (mode === 'edit') ? '' : 'none';
  document.getElementById('editBtn').style.display = (mode === 'view') ? '' : 'none';
  document.getElementById('updateBtn').style.display = (mode === 'edit' && !!accountSettings.settings) ? '' : 'none';
}

// Toast & Spinner (same as before)
function showSpinner() { document.getElementById('spinner-overlay')?.style?.setProperty('display', 'flex'); }
function hideSpinner() { document.getElementById('spinner-overlay')?.style?.setProperty('display', 'none'); }
...
// Autofill profile and account settings
async function autoFillProfile() {
  ...
  try {
    ...
    // Fetch account settings and set mode
...
async function autoFillAccountSettings(token) {
  try {
    const resp = await fetch(`${API_BASE}/account-settings`, { headers: { "Authorization": "Bearer " + token } });
    if (!resp.ok) return setFormMode('edit');
    const data = await resp.json();
    ...
    if (data && data.settings) {
      ...
      // Show ID/Proof docs info if present
      document.getElementById('idDocInfo').textContent = data.settings.idDocument ? "Uploaded" : "";
      document.getElementById('addrDocInfo').textContent = data.settings.proofOfAddress ? "Uploaded" : "";
      mode = 'view';
      setFormMode('view');
    } else {
      mode = 'edit';
...
document.getElementById('updateBtn').addEventListener('click', async () => {
  // Just submit the form as update
  document.getElementById('settingsForm').dispatchEvent(new Event('submit', {cancelable: true, bubbles: true}));
});
...
// Submit handler for Save and Update
document.getElementById('settingsForm').onsubmit = async function(e) {
  ...
  let idDocumentUrl = accountSettings.settings?.idDocument || "";
  let proofOfAddressUrl = accountSettings.settings?.proofOfAddress || "";
  try {
  ...
  // Send data to /api/account-settings (POST for both create & update)
  ...
  try {
    ...
    if (resp.ok && data.success) {
      ...
      // Switch to view mode after save/update
      mode = 'view';
...
async function uploadToCloudinary(file) {
  ...
  const resp = await fetch(`${API_BASE}/images`, { method: "POST", body: formData });
  const data = await resp.json();
