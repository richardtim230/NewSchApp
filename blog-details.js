...
async function likePost(postId) {
  const res = await fetch(`${READS_API}/${postId}/like`, { method: "PATCH" });
  return res.ok ? (await res.json()).likes : null;
}
async function countRead(postId) {
  try {
    await fetch(`${READS_API}/${postId}/read`, { method: "PATCH" });
  } catch {}
}
...
function attachCommentActionListeners() {
  document.querySelectorAll('.like-btn[data-comment-id]').forEach(btn => {
    btn.onclick = async function(e) {
      ...
      // Like comment endpoint can be implemented similarly
      this.disabled = false;
    }
  });
    ...
    <div class="relative w-full mb-4">
      ...
      <div class="absolute top-3 right-4 flex gap-2">
        <button id="likePostBtn" class="bg-white/90 rounded-full border border-yellow-200 px-3 py-1 flex items-center gap-1 font-semibold like-btn transition hover:bg-yellow-50">
          <svg class="w-5 h-5 text-yellow-500" fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902..."/></svg>
...
function injectBlogPostJsonLD({ title, image, author, datePublished, description }) {
  ...
  const jsonLd = {
    ...
    "publisher": {
      "@type": "Organization",
      "name": "OAU ExamGuard",
      "logo": { "@type": "ImageObject", "url": "https://oau.examguard.com.ng/logo.png" }
    },
